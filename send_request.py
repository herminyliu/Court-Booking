...
def findtime(court_id: str, book_date: str, session: str, time_no: str, court_num: str):
    ...
    # 这里返回的stock_id仅仅是该天该场地类型的第一个时段的stock_id！因此stock_id不能从这个请求的返回中提取出来
    return jsessionid_finddate


def findprice(court_id: str, book_date: str, session: str, jsessionid: str, time_no: str):
    # findprice这一步不可省略，因为我们需要获得jsessionid和预定日期特定的时段的stock_id
    ...
    stock_id = "000000"
    # 提取所需信息
    stock_id = data["object"]["stockList"][0]["id"]

    if stock_id == "000000":
        raise Exception(f"++++findtime函数中的stock_id未找到！请检查输入的日期时段是否有效！json文件提取出的id为：{id_value}++++")
    else:
        print(f'找到的stock_id为：{stock_id}')
    # 这里返回的stock_id仅仅是该天该场地类型的第一个时段的stock_id！因此stock_id不能从这个请求的返回中提取出来
    return jsessionid_findprice, stock_id
...
def findseat(court_id: str, session: str, jsessionid: str, stock_id: str, court_num: int):
    ...
    stock_detailid = raw_stock_detailid.split(',')[court_num-1].split('_')[1]  # 场地编号court_num从1开始
...
def get_yzm_image(session: str, jsessionid: str):
    ...
    if r.status_code == 200:
        ...
        raise Exception("获得验证码图片请求失败!请检查是否在预定时段内（8：40-21：40）。并且是否提供了有效的登录session！状态码为：" + str(r.status_code))
...
def book(court_id: str, session: str, jsessionid: str, stock_id: str, stockdetail_ids: str, x_index: int, count_try: int):
    ...
    if data['message'] == '未支付':
        print('预定成功！')
    elif data['message'] == '已被预定':  # 占位符，目前还不清楚被抢定了具体是什么界面
        raise AssertionError('场地已被预定')
    else:
...
def search_remaining_court():
    # TODO 在发现计划预订的场地被订后，搜索给定日期时段还未被预定的场地
    remain_status = True
    court_num = 0  # 还剩余的场地的编号

    return court_num, remain_status
